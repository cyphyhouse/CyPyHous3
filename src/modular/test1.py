...
zdata = [paddata([float(line.strip().split(',')[3]) for line in open('positions'+str(i)).readlines()],a) for i in range(start,end+1)]

#alldata = list(zip(xdata,ydata,zdata))
alldata = list(zip(xdata,ydata,zdata))
surfacedata = list(zip(xdata,ydata))
tasks = getTasks()


lines = [ax.plot(data[0],data[1],"lightgrey")[0] for data in alldata]

points = [ax.plot(data[0],data[1],"green",marker='2',ms=10)[0] for data in alldata]
...
def update(i):
    ...
    for j,line in enumerate(lines):
        ...
        if i <= 1500:
            ...
            #line.set_3d_properties(np.zeros(i))
            line.set_3d_properties(alldata[j][2][:i])

        else:
            line.set_data(alldata[j][0][i-1500:i], alldata[j][1][i-1500:i])
            #line.set_3d_properties(np.zeros(20))
            #line.set_3d_properties(z_traj[i-20:i])
            line.set_3d_properties(alldata[j][2][i-1500:i])


    for j,point in enumerate(points):
        point.set_data(alldata[j][0][i],alldata[j][1][i])
        point.set_3d_properties(alldata[j][2][i])
...
ax.set_xlim3d([-10.0, 250.0])
ax.set_xlabel('X')

ax.set_ylim3d([-10.0, 250.0])
ax.set_ylabel('Y')
