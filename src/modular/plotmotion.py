...
timeplots = [ax.plot(tdata,x,"grey")[0] for x in xdata]
pointtimeplots = [ax.plot(tdata,x,"darkgreen",marker='2',ms=10)[0] for x in xdata]
#plt.show()
...
def init():  # only required for blitting to give a clean slate.
    ...
    return lines + pointtimeplots

def update(i):
    for j,tp in enumerate(timeplots):
        tp.set_data(tdata[:i],xdata[j][:i])

    for j,point in enumerate(pointtimeplots):
        try:
            point.set_data(tdata[i],xdata[j][i])
        except:
            pass
    '''
    for j,line in enumerate(lines):
        ...
        if i <= 600:
           ...
           line.set_data(alldata[j][0][i-600:i], alldata[j][1][i-600:i])
