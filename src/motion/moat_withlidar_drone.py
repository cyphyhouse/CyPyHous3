...
class MoatWithLidarDrone(MoatTestDrone):

    def __init__(self, config):
        super(MoatWithLidarDrone, self).__init__(config)
        self.tsync = queue.Queue()

        sub_scan = message_filters.Subscriber(config.rospy_node.split("/")[0] + '/laser/scan',
                                              LaserScan)
        # sub_scan = message_filters.Subscriber('/drone1/laser/scan',
                                            #   LaserScan)
        ...
        print("config.rospy_node is ", config.rospy_node.split("/")[0])
    ...
    def _get_scan_at_pos(self, scan, pos) -> None:
        yaw, pitch, roll = self._quaternion_to_euler(pos.pose.orientation)
        position = Pos(np.array([pos.pose.position.x, pos.pose.position.y, pos.pose.position.z, yaw]))
        
        # Drop detection if pitch or roll angle is too sharp
        if pitch >= 0.1 or pitch <= -0.1 or roll >= 0.1 or roll <= -0.1:
            return
