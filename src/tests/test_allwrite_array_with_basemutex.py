...
class AddNums(AgentThread):
    ...
    def initialize_vars(self):
        ...
        self.create_ar_var('cur_sum', int, self.pid())
    ...
    def loop_body(self):
        if not self.locals['added']:
            if not self.lock('adding'):
                return
            old_sum = self.read_from_shared('sum', None)
            self.write_to_shared('sum', None, old_sum + self.pid())
            self.write_to_shared('cur_sum', self.pid(), old_sum + self.pid())  # Set equal to sum
        ...
        if self.read_from_shared('numadded', None) == self.num_agents():
            ...
            self.write_to_shared('cur_sum', self.pid(), self.locals['finalsum'])  # Set equal to finalsum
            self.stop()
            return
...
class AllwriteTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.agent_list = [AddNums(c1), AddNums(c2), AddNums(c3), AddNums(c4), AddNums(c5)]

    def test_write_to_shared(self):
        time.sleep(5)
        for agent in self.agent_list:
            finalsum = agent.locals['finalsum']
            self.assertEqual(finalsum, 10)
            numadded = agent.read_from_shared('numadded', None)
            self.assertEqual(numadded, len(self.agent_list))
            print("agent", agent.pid(), "cur_sum", [agent.read_from_shared('cur_sum', i) for i in range(0,5)])
