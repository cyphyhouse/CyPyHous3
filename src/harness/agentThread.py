...
from src.harness.msg_create import round_update_msg_create, stop_msg_create, init_msg_create
...
class AgentThread(ABC, Thread):
    ...
    def create_ar_var(self, name, data_type, initial_value=None):
        self.agent_gvh.create_ar_var(name, data_type, initial_value)
        pass

    def create_aw_var(self, name, data_type, initial_value=None):
        self.agent_gvh.create_aw_var(name, data_type, initial_value)
        pass

    def initialize_lock(self, key):
        self.baselocks[key] = BaseMutex(key)
    ...
    def read_from_shared(self, var_name: str, index: Union[int, None]) -> Union[int,]:
        """
    ...
    def run(self) -> None:
        ...
        while not self.agent_gvh.init and not self.stopped():
            self.initialize_vars()
            self.msg_handle()
            self.agent_gvh.send(init_msg)
            time.sleep(0.05)
            ...
            try:
                ...
                while not self.agent_gvh.update_round:
                    if self.stopped():
                        break
                    self.msg_handle()
                    self.agent_gvh.send(round_update_msg)
                    time.sleep(0.1)
